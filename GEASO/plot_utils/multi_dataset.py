        ...
        for j, mapping in enumerate(self.mappings):
            ...
            for i, (layer, anno) in enumerate(zip(self.loc_list[j:j + 2], self.anno_list[j:j + 2])):
                ...
                for cell_type in self.celltypes:
                    ...
                    ys = slice[:, 1]
                    zs = height * (j + i)
                    ax.scatter(xs, ys, zs, s=point_size[i], c=c_map[cell_type], alpha=point_alpha, rasterized=True)
            # plot mapping line
    ...
    def draw_3D(self,
        ...
        if self.meta:
            ...
            for i, dataset in enumerate(self.datasets):
                ...
                for cell_type in self.celltypes:
                    ...
                    zs = i
                    if self.expr:
                        ax.scatter(xs, ys, zs, s=point_size[i], c=slice[self.expr], cmap=c_map, norm=norm, rasterized=True)
                    else:
                        ax.scatter(xs, ys, zs, s=point_size[i], c=c_map[cell_type], rasterized=True)
        # plot points without meta
            ...
            for i, dataset in enumerate(self.datasets):
                ...
                ys = dataset['y']
                zs = i
                ax.scatter(xs, ys, zs, s=point_size[i], rasterized=True)
        # plot line
        self.c_map = c_map
...
class match_3D_multi_error(match_3D_multi):
    ...
    def draw_lines(self, ax, show_error, show_celltype, default_color, line_width=0.3, line_alpha=0.7) -> None:
        for i in range(self.matching.shape[1]):
            ...
            point1 = np.append(self.dataset_B[self.dataset_B['index'] == pair[0]][['x', 'y']], 1)
            coord = np.row_stack((point0, point1))
            ax.scatter(point0[0], point0[1], point0[2], color='red', alpha=1, s=0.3, rasterized=True)
            ax.scatter(point1[0], point1[1], point1[2], color='red', alpha=1, s=0.3, rasterized=True)
...
class match_3D_celltype(match_3D_multi):
    ...
    def __init__(self,
        ...
        super().__init__(dataset_A, dataset_B, matching, meta, expr, subsample_size, reliability, scale_coordinate,
                         rotate, exchange_xy, subset)
    ...
    def draw_lines(self, ax, show_error, show_celltype, default_color, line_width: float = 0.3,
        ...
        for i in range(self.matching.shape[1]):
            ...
            if self.highlight_cell:
                ax.scatter(point0[0], point0[1], point0[2], color=self.highlight_cell, alpha=1, s=1, rasterized=True)
                ax.scatter(point1[0], point1[1], point1[2], color=self.highlight_cell, alpha=1, s=1, rasterized=True)
